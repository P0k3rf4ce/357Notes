...
\end{question}
\begin{proof}
Take a basis $\beta$ for $\R$ as a vector space over $\Q$. This gives us a unique expression of any real number $x$ as $q_1e_1 + \cdots + q_n e_n$ for $q_i \in \Q \setminus \{0\}, e_i \in \beta$. This lets us define $f$ by $q_1e_1 + \cdots + q_n e_n \mapsto q_1 + \cdots + q_n$. This function is obviously additive, but it cannot be continuous on $\R$, since its image is the disconnected set $\Q$. Hence it is not linear.
\end{proof}
...
\end{question}
\begin{proof}
The result of Lusin's theorem is that $f = g$ on $F$ for a continuous function $g: \R \to \R$. Since $g$ is continuous on $F$, then so must be $f$. $f$ as a function on $E$ might not be continuous at any point however. As an example, consider the simple function $f = \chi_{\Q \cap [0, 1]}: [0, 1] \to \R$ which is discontinuous everywhere on $[0, 1]$, but by Lusin's theorem can be restricted to a continuous function.
\end{proof}
...
\end{question}
\begin{proof}
Consider the sequence of functions $f_n: \R \to \R: x \mapsto x/n$ which converges pointwise to the zero function. For any set $F \subseteq \R$, in order to have $m(\R \setminus F) < 1$, we must have that $F$ is unbounded. For any $N > 0$, we may find $x \in F \setminus (-N, N)$ for which we have $|f_N(x)| = |x/N| \geq 1$, so the convergence $f_n \to f$ cannot be uniform.
\end{proof}
...
\end{question}
\begin{proof}
Apply Lusin's theorem for domains of finite measure to get for each $n \in \Z$ a continuous $g_n: \R \to \R$ and a closed set $F_n \subseteq (n, n + 1) \cap E$ with $m(E \cap (n, n + 1) \setminus F_n) < \varepsilon/(3 \cdot 2^{|n|})$ for which $f = g_n$ on $F_n$. One can then show that with this construction, $F := \bigcup_{n \in \Z} F_n$ is closed, and we have 
\[ m(E \setminus F) = \sum_{n \in \Z} m(E \cap (n, n + 1) \setminus F_n) < \sum_{n \in \Z} \frac{\varepsilon}{3 \cdot 2^{|n|}} = \varepsilon \]
We then note that each $F_n$ is compact, so $a_n := \operatorname{dist}(n, F_n), b_n := \operatorname{dist}(n + 1, F_n)$ are nonzero. This allows us to construct a continuous function $g: \R \to \R$ by defining it to be $g_n$ on $[n + a_n, n + 1 - b_n]$ and extending it linearly everywhere else. By construction, $g = f$ on $F$, as required.
\end{proof}
\end{document}